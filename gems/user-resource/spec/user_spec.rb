...
RSpec.describe User do
  ...
  it "gets user counts for every requested account" do
    response = instance_double(Faraday::Response, status: 200, body: '{"account-1":2}')
    expect(Faraday).to receive(:post).with("http://user-service/accounts/users/counts")
      .and_yield(request).and_return(response)

    result = AuthorizationContext.as_requesting_user(user_id: "actor") do
      described_class.users_count(["account-1"])
    end
    expect(result).to eq("account-1": 2)
  ...
  it "searches through the POST read endpoint" do
    ...
    response = instance_double(response_class, body: '[{"id":"user-1"}]')
    allow(described_class).to receive(:connection).and_return(connection)
    expect(connection).to receive(:post).with(
      "/users/search", '{"account_id":["account-1"]}',
      hash_including("pad-user-id" => "actor", "Content-Type" => "application/json")
    ).and_return(response)

    result = AuthorizationContext.as_requesting_user(user_id: "actor") do
      described_class.search(account_id: ["account-1"])
    end
    expect(result.map(&:id)).to eq(["user-1"])
  end
end
