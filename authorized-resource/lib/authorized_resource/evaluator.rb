  ...
  module Evaluator
    ...
    def authorize!(resource_class, kind, records, operation: kind, requirements: nil)
      ...
      if context.iam?
        ...
        unless allowed.include?(context.iam_identity)
          raise AuthorizedResource::AuthorizationDenied,
            "#{context.iam_identity} is not authorized for #{resource_class.name} #{operation}"
        end
        return records
      end
      ...
      if requirements.empty?
        raise AuthorizedResource::AuthorizationDenied,
          "requesting users are not authorized for #{resource_class.name} #{operation}"
      end
      ...
      AuthorizedResource::Instrumentation.trace_authorization(resource_class, operation, records, targets) do
        ...
        raise AuthorizedResource::AuthorizationDenied,
          "authorization denied for #{resource_class.name} #{operation}" if denied
      end
      records
    end
    ...
    def ensure_policy!(resource_class, policy, kind)
      unless policy.configured_for?(kind)
        raise AuthorizedResource::PolicyConfigurationError,
          "#{resource_class.name} has no explicit #{kind} authorization policy"
      end
      if kind == :modify && policy.read_only?
        raise AuthorizedResource::ReadOnlyError, "#{resource_class.name} is explicitly read-only"
      end
    end
  end
end
